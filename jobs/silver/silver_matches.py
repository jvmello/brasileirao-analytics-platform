...
def read_bronze_matches(spark: SparkSession, config: AppConfig) -> DataFrame:
    bronze_path = f"s3a://{config.bucket_name}/{config.bronze_raw_prefix.rstrip('/')}/source=matches/"
...
def main() -> None:
    ...
    try:
        bronze_matches = read_bronze_matches(spark, config)
