...
def transform_match_statistics(
    raw_df: DataFrame, silver_matches_df: DataFrame
) -> DataFrame:
    ...
    possession_col = resolve_column(
        raw_df, "posse_de_bola", "posse de bola", "possession"
    )
    ...
    yellow_cards_col = resolve_column(
        raw_df, "cartao_amarelo", "cartão_amarelo", "yellow_cards"
    )
    red_cards_col = resolve_column(
        raw_df, "cartao_vermelho", "cartão_vermelho", "red_cards"
    )
    ...
    checks.append(
        (
            ...
            df.filter(
                ...
                & (
                    F.when(
                        F.col("goals_scored") > F.col("goals_conceded"), F.lit("win")
                    )
                    .when(
                        F.col("goals_scored") < F.col("goals_conceded"), F.lit("loss")
                    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build silver.match_statistics from bronze raw match_statistics."
    )
    parser.add_argument(
        "--load-date",
        required=False,
...
def main() -> None:
    ...
    try:
        ...
        print(
            f"distinct_matches={silver_match_statistics.select('match_id').distinct().count()}"
        )
        print(
            f"distinct_seasons={silver_match_statistics.select('season').distinct().count()}"
        )
